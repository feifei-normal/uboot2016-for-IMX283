		       ...
		       int vid_hdr_offset, int max_beb_per1024)
{
	struct ubi_device *ubi;
	int i, err, ref = 0;
	printf("Dug ubi_attach_mtd_dev %d\n",max_beb_per1024);
	...
	if (ubi_num == UBI_DEV_NUM_AUTO) {
		...
		if (ubi_num >= UBI_MAX_DEVICES)
			{
			printf("ubi_num >= UBI_MAX_DEVICES\n");
			return -EINVAL;
			}
...
printf("misc_register\n");
	...
	for (i = 0; i < mtd_devs; i++) {
		...
		printf("ubi_attach_mtd_dev %d\n",err);
		mutex_unlock(&ubi_devices_mutex);
		if (err < 0) {
			printf("UBI error: cannot attach mtd%d\n",
			       mtd->index);
			put_mtd_device(mtd);
